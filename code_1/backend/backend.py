...
from ConnectDatabase_v2 import get_data_from_database
import datetime
import csv
import pyodbc
import subprocess
...
def ShowPanel():
    ...
    result1 = get_data_from_database(query1)  # 当天
    result2 = get_data_from_database(query2)  # 前一天
    result3 = get_data_from_database(query3)  # 后一天
...
def ShowPatientRecord():
    ...
    history = proccess_result_for_ShowPatientRecord(get_data_from_database(query1))
    future = proccess_result_for_ShowPatientRecord(get_data_from_database(query2))
    # print(result)
    return jsonify({'history':history, 'future':future})
